...
do n = 0,ndom-1
  ...
  if (WT > GlobCoord(minz)  .AND.  WT < GlobCoord(maxz)) then
     Tdomain%Sub_domain(n)%midStress1 = Tdomain%Sub_domain(n)%midStress1+ &
                                    9.8* 1000.0* (WT-GlobCoord(minz))


! write(99,*) n, Tdomain%Sub_domain(n)%midStress1, WT, GlobCoord(minz), GlobCoord(maxz)


  endif
!   write(99,*)  'WT', WT

! write(99,*) n, Tdomain%Sub_domain(n)%midStress1, rho, thickness
...
! do n=0,ndom
! write(99,*) n, Tdomain%Sub_domain(n)%midStress1, Tdomain%Sub_domain(n)%midStress2
! enddo



! IF NO SATURATION CONSIDERATION
...
!!!
      ...
      if (.not.  Tdomain%Sub_domain(nummat)%Eeffective) then
        ...
        gammaref = (m1(numsat)* Peff0(i) + cohesion(numsat))/ Gm0(i)   
...
!         write(99,*) n, i, Enonli%specel(n)%gammaref(i), Peff0(i), Tdomain%Sub_domain(nummat)%midStress2
      endif
